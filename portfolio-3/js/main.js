/* ========================================
   O bag Japan - Portfolio 3: Soft & Warm
   Main JS
   ======================================== */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (hamburger && mobileNav) {
    ...
    // Close when clicking backdrop (::before pseudo-element triggers click on mobileNav itself)
  ...
  function goToSlide(index) {
    ...
    if (content) {
      const animated = content.querySelectorAll('.hero-eyebrow, .hero-title, .hero-subtitle, .btn');
      animated.forEach(el => {
  ...
  const fadeElements = document.querySelectorAll(
    '.section-eyebrow, .section-title, .section-text, .step-card, .product-card, .part-card, .split-content, .split-image, .newsletter-inner, .instagram-item, .value-item'
  );
